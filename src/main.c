...
/* ============================================================================
 * The Source (src) programming language
 * ============================================================================ */
...
#define MAX_STRUCT_TYPES 100
#define MAX_STRUCT_FIELDS 50
#define MAX_ERROR_HANDLERS 50
...
typedef struct {
    ...
    char currentFile[MAX_PATH_LENGTH];  // Track currently executing file
} ImportTracker;
...
typedef struct {
    ErrorValue errors[MAX_ERROR_HANDLERS];
    int count;
    bool inTryBlock;
    int tryDepth;
} ErrorSystem;
...
typedef enum {
    ...
    TOKEN_SPAWN,
    TOKEN_AT,           // @ symbol
    TOKEN_REAL_TIME,    // real_time keyword
    TOKEN_GLOBAL,       // global keyword for global variables
    // NEW TOKENS FOR ENHANCED FEATURES
    TOKEN_STRUCT,       // struct keyword
    TOKEN_FILE,          // file type
    TOKEN_OPEN,          // open function
    TOKEN_CLOSE,         // close method
    TOKEN_READ,          // read method
    TOKEN_WRITE,         // write method
    TOKEN_READLN,        // readln method
    TOKEN_WRITELN,       // writeln method
    TOKEN_ERROR,         // error type
    TOKEN_TRY,           // try keyword
    TOKEN_CATCH,         // catch keyword
    TOKEN_THROW,         // throw keyword
    TOKEN_FORMAT,        // format function
    TOKEN_TUPLE,         // tuple type
} TokenType;

/* ============================================================================
 * NEW: Struct Type System (AFTER TokenType definition)
 * ============================================================================ */
...
typedef struct {
    StructType types[MAX_STRUCT_TYPES];
    int count;
} StructRegistry;

static StructRegistry structRegistry;
...
typedef enum {
    ...
    VAL_STRING,
    VAL_LIST,
    VAL_STRUCT,      // NEW: struct instances
    VAL_FILE,        // NEW: file handles
    VAL_ERROR,       // NEW: error values
    VAL_TUPLE,       // NEW: tuple values
} ValueType;
...
struct Value {
    ...
    union {
        ...
        List listVal;
        StructInstance structVal;  // NEW
        FileHandle fileVal;        // NEW
        ErrorValue errorVal;       // NEW
        Tuple tupleVal;           // NEW
    } as;
};
...
typedef enum {
    ...
    EXPR_RANGE,
    EXPR_COMPTIME,
    EXPR_STRUCT_INIT,   // NEW: struct initialization
    EXPR_FIELD_ACCESS,  // NEW: struct.field
    EXPR_TUPLE,         // NEW: tuple literal
} ExprType;
...
// NEW: Struct initialization expression
typedef struct {
    char typeName[256];
    char fieldNames[MAX_STRUCT_FIELDS][256];
    Expr *fieldValues[MAX_STRUCT_FIELDS];
    int fieldCount;
} StructInitExpr;

// NEW: Field access expression (struct.field)
typedef struct {
    Expr *object;
    char fieldName[256];
} FieldAccessExpr;

// NEW: Tuple expression
typedef struct {
    Expr **elements;
    int count;
} TupleExpr;
...
struct Expr {
    ...
    union {
        ...
        StructInitExpr structInit;  // NEW
        FieldAccessExpr fieldAccess; // NEW
        TupleExpr tuple;            // NEW
    } as;
};
...
typedef enum {
    ...
    STMT_IMPORT,
    STMT_SPAWN,
    STMT_INDEX_ASSIGN,  // list[index] = value
    STMT_STRUCT_DEF,    // NEW: struct definition
    STMT_FIELD_ASSIGN,  // NEW: struct.field = value
    STMT_TRY_CATCH,     // NEW: try-catch block
    STMT_THROW,         // NEW: throw error
    STMT_DESTRUCT,      // NEW: destructuring assignment
} StmtType;
...
typedef struct {
    ...
    Expr *initializer;
    bool isComptime;
    bool isGlobal;  // Global variable flag
    char structType[256];  // NEW: for struct variables
} VarDeclStmt;
...
typedef struct {
    ...
    Expr *index;
    Expr *value;
} IndexAssignStmt;  // for list[index] = value

// NEW: Struct definition statement
typedef struct {
    char name[256];
    char fieldNames[MAX_STRUCT_FIELDS][256];
    TokenType fieldTypes[MAX_STRUCT_FIELDS];
    char fieldStructTypes[MAX_STRUCT_FIELDS][256];  // For nested structs
    int fieldCount;
} StructDefStmt;

// NEW: Field assignment (struct.field = value)
typedef struct {
    char objectName[256];
    char fieldName[256];
    Expr *value;
...
typedef struct {
    char varNames[MAX_PARAMS][256];
    int varCount;
    Expr *value;  // Should be a tuple or list
} DestructStmt;
...
struct Stmt {
    ...
    union {
        ...
        ImportStmt import;
        SpawnStmt spawn;
        IndexAssignStmt indexAssign;  // list[index] = value
        StructDefStmt structDef;      // NEW
        FieldAssignStmt fieldAssign;  // NEW
        TryCatchStmt tryCatch;        // NEW
        ThrowStmt throwStmt;          // NEW
        DestructStmt destruct;        // NEW
    } as;
};
...
typedef struct {
    ...
    char paramStructTypes[MAX_PARAMS][256];  // NEW: for struct parameters
    int paramCount;
    Stmt *body;
    char definedInFile[MAX_PATH_LENGTH];  // Track which file defined this function
} Function;
...
StructType *findStructType(const char *name);  // NEW
...
/* ============================================================================
 * NEW: Error Handling System Implementation
 * ============================================================================ */

void initErrorSystem() {
    errorSystem.count = 0;
    errorSystem.inTryBlock = false;
    errorSystem.tryDepth = 0;
}

void throwError(const char *type, const char *message, int code) {
    if (errorSystem.inTryBlock) {
        // Create error value for catch block
        if (errorSystem.count < MAX_ERROR_HANDLERS) {
            ErrorValue *err = &errorSystem.errors[errorSystem.count++];
            strncpy(err->type, type, 255);
            err->type[255] = '\0';
            strncpy(err->message, message, MAX_STRING_LENGTH - 1);
            err->message[MAX_STRING_LENGTH - 1] = '\0';
            err->code = code;
            err->active = true;
        }
    } else {
        // Uncaught error - terminate
        fprintf(stderr, "\n============================================\n");
...
Value createErrorValue(const char *type, const char *message, int code) {
    Value v;
    v.type = VAL_ERROR;
    strncpy(v.as.errorVal.type, type, 255);
    v.as.errorVal.type[255] = '\0';
    strncpy(v.as.errorVal.message, message, MAX_STRING_LENGTH - 1);
    v.as.errorVal.message[MAX_STRING_LENGTH - 1] = '\0';
    v.as.errorVal.code = code;
    v.as.errorVal.active = true;
    return v;
}

/* ============================================================================
 * NEW: Struct Registry Implementation
 * ============================================================================ */

void initStructRegistry() {
    structRegistry.count = 0;
}

void registerStructType(StructType *structType) {
    if (structRegistry.count < MAX_STRUCT_TYPES) {
        structRegistry.types[structRegistry.count++] = *structType;
    }
}

StructType *findStructType(const char *name) {
    for (int i = 0; i < structRegistry.count; i++) {
        if (strcmp(structRegistry.types[i].name, name) == 0) {
            return &structRegistry.types[i];
        }
    }
    return NULL;
}
...
void initImportTracker() {
    ...
    importTracker.currentFile[0] = '\0';  // No file initially
...
void processImport(const char *filename) {
    ...
    char savedFile[MAX_PATH_LENGTH];
    strcpy(savedFile, importTracker.currentFile);
    ...
    strcpy(importTracker.currentFile, savedFile);
}
...
// NEW: Create struct instance
Value createStructValue(const char *typeName) {
    StructType *type = findStructType(typeName);
    if (!type) {
        fprintf(stderr, "Runtime Error: Unknown struct type '%s'\n", typeName);
        exit(1);
    }
    
    Value v;
    v.type = VAL_STRUCT;
    strncpy(v.as.structVal.typeName, typeName, 255);
    v.as.structVal.typeName[255] = '\0';
    v.as.structVal.fieldCount = type->fieldCount;
    v.as.structVal.fields = calloc(type->fieldCount, sizeof(Value));
    
    // Initialize fields with default values
    for (int i = 0; i < type->fieldCount; i++) {
        v.as.structVal.fields[i] = createIntValue(0);
    }
    
    return v;
}

// NEW: Create file value
Value createFileValue(const char *path, const char *mode) {
    Value v;
    v.type = VAL_FILE;
    strncpy(v.as.fileVal.path, path, MAX_PATH_LENGTH - 1);
    v.as.fileVal.path[MAX_PATH_LENGTH - 1] = '\0';
    strncpy(v.as.fileVal.mode, mode, 7);
    v.as.fileVal.mode[7] = '\0';
    v.as.fileVal.handle = fopen(path, mode);
    v.as.fileVal.isOpen = (v.as.fileVal.handle != NULL);
    
    if (!v.as.fileVal.isOpen) {
        char errMsg[512];
        snprintf(errMsg, sizeof(errMsg), "Failed to open file '%s' with mode '%s': %s", 
                 path, mode, strerror(errno));
        throwError("FileError", errMsg, errno);
    }
    
    return v;
}

// NEW: Create tuple value
Value createTupleValue(Value *elements, int count) {
    Value v;
    v.type = VAL_TUPLE;
    v.as.tupleVal.count = count;
    v.as.tupleVal.elements = calloc(count, sizeof(Value));
    for (int i = 0; i < count; i++) {
        v.as.tupleVal.elements[i] = elements[i];
    }
    return v;
}
...
// NEW: Get struct field by name (also handles error fields)
Value *getStructField(Value *structVal, const char *fieldName) {
    // Handle error type fields
    if (structVal->type == VAL_ERROR) {
        static Value tempVal;  // Static to return pointer
        if (strcmp(fieldName, "message") == 0) {
            tempVal = createStringValue(structVal->as.errorVal.message);
            return &tempVal;
        } else if (strcmp(fieldName, "type") == 0) {
            tempVal = createStringValue(structVal->as.errorVal.type);
            return &tempVal;
        } else if (strcmp(fieldName, "code") == 0) {
            tempVal = createIntValue(structVal->as.errorVal.code);
            return &tempVal;
        } else {
            fprintf(stderr, "Runtime Error: Unknown field '%s' in error value\n", fieldName);
            exit(1);
        }
    }
    
    if (structVal->type != VAL_STRUCT) {
        fprintf(stderr, "Runtime Error: Cannot access field on non-struct\n");
        exit(1);
    }
    
    StructType *type = findStructType(structVal->as.structVal.typeName);
    if (!type) {
        fprintf(stderr, "Runtime Error: Unknown struct type\n");
        exit(1);
    }
    
    for (int i = 0; i < type->fieldCount; i++) {
        if (strcmp(type->fields[i].name, fieldName) == 0) {
            return &structVal->as.structVal.fields[i];
        }
    }
    
    fprintf(stderr, "Runtime Error: Unknown field '%s' in struct '%s'\n", 
            fieldName, structVal->as.structVal.typeName);
    exit(1);
}
...
void printValue(Value val) {
    ...
    switch (val.type) {
        ...
        case VAL_LIST:
            ...
            break;
        case VAL_STRUCT: {
            StructType *type = findStructType(val.as.structVal.typeName);
            printf("%s{", val.as.structVal.typeName);
            for (int i = 0; i < val.as.structVal.fieldCount; i++) {
                if (type) printf("%s: ", type->fields[i].name);
                printValue(val.as.structVal.fields[i]);
                if (i < val.as.structVal.fieldCount - 1) printf(", ");
            }
            printf("}");
            break;
        }
        case VAL_FILE:
            printf("File<%s, %s>", val.as.fileVal.path, 
                   val.as.fileVal.isOpen ? "open" : "closed");
            break;
        case VAL_ERROR:
            printf("Error{type: %s, message: %s, code: %d}", 
                   val.as.errorVal.type, val.as.errorVal.message, val.as.errorVal.code);
            break;
        case VAL_TUPLE:
            printf("(");
            for (int i = 0; i < val.as.tupleVal.count; i++) {
                printValue(val.as.tupleVal.elements[i]);
                if (i < val.as.tupleVal.count - 1) printf(", ");
            }
            printf(")");
            break;
    }
...
void scanIdentifier(Lexer *lexer) {
    ...
    else if (strcmp(id, "struct") == 0) type = TOKEN_STRUCT;
    else if (strcmp(id, "file") == 0) type = TOKEN_FILE;
    else if (strcmp(id, "open") == 0) type = TOKEN_OPEN;
    else if (strcmp(id, "close") == 0) type = TOKEN_CLOSE;
    else if (strcmp(id, "read") == 0) type = TOKEN_READ;
    else if (strcmp(id, "write") == 0) type = TOKEN_WRITE;
    else if (strcmp(id, "readln") == 0) type = TOKEN_READLN;
    else if (strcmp(id, "writeln") == 0) type = TOKEN_WRITELN;
    else if (strcmp(id, "error") == 0) type = TOKEN_ERROR;
    else if (strcmp(id, "try") == 0) type = TOKEN_TRY;
    else if (strcmp(id, "catch") == 0) type = TOKEN_CATCH;
    else if (strcmp(id, "throw") == 0) type = TOKEN_THROW;
    else if (strcmp(id, "format") == 0) type = TOKEN_FORMAT;
    else if (strcmp(id, "tuple") == 0) type = TOKEN_TUPLE;
...
Expr *parsePrimary(Parser *parser) {
    ...
    // NEW: error{type, message, code} for creating error values
    if (matchToken(parser, TOKEN_ERROR)) {
        consume(parser, TOKEN_LBRACE, "Expected '{' after 'error'");
        
        // Parse: error{type, message, code}
        Value typeVal = evaluateExpr(parseExpression(parser));
        consume(parser, TOKEN_COMMA, "Expected ',' after error type");
        Value msgVal = evaluateExpr(parseExpression(parser));
        consume(parser, TOKEN_COMMA, "Expected ',' after error message");
        Value codeVal = evaluateExpr(parseExpression(parser));
        consume(parser, TOKEN_RBRACE, "Expected '}' after error code");
        
        if (typeVal.type != VAL_STRING || msgVal.type != VAL_STRING || codeVal.type != VAL_INT) {
            fprintf(stderr, "Parser Error: error{} requires (string, string, int)\n");
            exit(1);
        }
        
        return createLiteralExpr(createErrorValue(typeVal.as.stringVal, msgVal.as.stringVal, codeVal.as.intVal));
    }
    ...
    if (matchToken(parser, TOKEN_IDENTIFIER)) {
        ...
        // NEW: Check for struct initialization: TypeName{field1: val1, ...}
        if (matchToken(parser, TOKEN_LBRACE)) {
            Expr *expr = malloc(sizeof(Expr));
            expr->type = EXPR_STRUCT_INIT;
            strcpy(expr->as.structInit.typeName, name);
            expr->as.structInit.fieldCount = 0;
            
            if (!check(parser, TOKEN_RBRACE)) {
                do {
                    Token *fieldName = advanceToken(parser);
                    if (expr->as.structInit.fieldCount < MAX_STRUCT_FIELDS) {
                        strcpy(expr->as.structInit.fieldNames[expr->as.structInit.fieldCount], 
                               fieldName->lexeme);
                        consume(parser, TOKEN_COLON, "Expected ':' after field name");
                        expr->as.structInit.fieldValues[expr->as.structInit.fieldCount] = 
                            parseExpression(parser);
                        expr->as.structInit.fieldCount++;
                    }
                } while (matchToken(parser, TOKEN_COMMA));
            }
            
            consume(parser, TOKEN_RBRACE, "Expected '}' after struct initializer");
            return expr;
        }
    ...
    if (matchToken(parser, TOKEN_LPAREN)) {
        // Could be either a grouped expression or a tuple
        ...
        if (matchToken(parser, TOKEN_COMMA)) {
            ...
            expr->as.tuple.elements = malloc(MAX_PARAMS * sizeof(Expr*));
            expr->as.tuple.elements[0] = first;
            expr->as.tuple.count = 1;
            
            do {
                expr->as.tuple.elements[expr->as.tuple.count++] = parseExpression(parser);
            } while (matchToken(parser, TOKEN_COMMA) && !check(parser, TOKEN_RPAREN));
    ...
    // NEW: format(string, args...) for string formatting
    if (matchToken(parser, TOKEN_FORMAT)) {
        consume(parser, TOKEN_LPAREN, "Expected '(' after 'format'");
        Expr *expr = malloc(sizeof(Expr));
        expr->type = EXPR_CALL;
        strcpy(expr->as.call.name, "format");
        expr->as.call.argCount = 0;
        expr->as.call.args = malloc(MAX_PARAMS * sizeof(Expr*));
        ...
        consume(parser, TOKEN_RPAREN, "Expected ')' after format arguments");
        return expr;
    }
    
    // NEW: open(path, mode) for file I/O
    if (matchToken(parser, TOKEN_OPEN)) {
        consume(parser, TOKEN_LPAREN, "Expected '(' after 'open'");
        Expr *expr = malloc(sizeof(Expr));
        expr->type = EXPR_CALL;
        strcpy(expr->as.call.name, "open");
        expr->as.call.argCount = 0;
        expr->as.call.args = malloc(MAX_PARAMS * sizeof(Expr*));
        ...
        consume(parser, TOKEN_RPAREN, "Expected ')' after open arguments");
        return expr;
    }
...
Expr *parsePostfix(Parser *parser) {
    ...
    while (true) {
        ...
        // NEW: Field access (struct.field) or method calls (file.read())
        else if (matchToken(parser, TOKEN_DOT)) {
            Token *fieldToken = advanceToken(parser);
            ...
            if (matchToken(parser, TOKEN_LPAREN)) {
                ...
                call->type = EXPR_CALL;
                // Encode method name with double underscore prefix
                snprintf(call->as.call.name, sizeof(call->as.call.name), 
                         "__%s", fieldToken->lexeme);
                call->as.call.argCount = 1;
                call->as.call.args = malloc(MAX_PARAMS * sizeof(Expr*));
                call->as.call.args[0] = expr;  // First arg is the object itself
                
                if (!check(parser, TOKEN_RPAREN)) {
                    do {
                        call->as.call.args[call->as.call.argCount++] = parseExpression(parser);
                    } while (matchToken(parser, TOKEN_COMMA));
                }
                ...
                // Field access
                Expr *field = malloc(sizeof(Expr));
                field->type = EXPR_FIELD_ACCESS;
                field->as.fieldAccess.object = expr;
                strcpy(field->as.fieldAccess.fieldName, fieldToken->lexeme);
                expr = field;
            }
        } else {
            break;
...
Stmt *parseStatement(Parser *parser) {
    ...
    // NEW: Try-catch block
    if (matchToken(parser, TOKEN_TRY)) {
        consume(parser, TOKEN_LBRACE, "Expected '{' after 'try'");
        Stmt *stmt = malloc(sizeof(Stmt));
        stmt->type = STMT_TRY_CATCH;
        stmt->as.tryCatch.tryBlock = parseBlock(parser);
        
        consume(parser, TOKEN_CATCH, "Expected 'catch' after try block");
        consume(parser, TOKEN_LPAREN, "Expected '(' after 'catch'");
        Token *errorVar = advanceToken(parser);
        strcpy(stmt->as.tryCatch.errorVar, errorVar->lexeme);
        consume(parser, TOKEN_RPAREN, "Expected ')' after error variable");
        consume(parser, TOKEN_LBRACE, "Expected '{' after catch");
        stmt->as.tryCatch.catchBlock = parseBlock(parser);
    ...
    if (matchToken(parser, TOKEN_THROW)) {
        ...
        stmt->as.throwStmt.errorExpr = parseExpression(parser);
        return stmt;
    }
    ...
    if (matchToken(parser, TOKEN_INT) || matchToken(parser, TOKEN_DOUBLE) || 
        ...
        matchToken(parser, TOKEN_LIST) || matchToken(parser, TOKEN_FILE) ||
        matchToken(parser, TOKEN_ERROR) || matchToken(parser, TOKEN_TUPLE)) {
        Stmt *stmt = malloc(sizeof(Stmt));
        stmt->type = STMT_VAR_DECL;
        stmt->as.varDecl.varType = previousToken(parser)->type;
        stmt->as.varDecl.isComptime = false;
        stmt->as.varDecl.isGlobal = isGlobal;
        stmt->as.varDecl.structType[0] = '\0';  // NEW: Initialize structType
    ...
    // NEW: Check for struct type variable declaration (e.g., Point p = ...)
    if (check(parser, TOKEN_IDENTIFIER)) {
        char possibleType[256];
        strcpy(possibleType, peekToken(parser)->lexeme);
        
        // Check if it's a registered struct type
        if (findStructType(possibleType) != NULL) {
            advanceToken(parser);  // consume type name
            
            Stmt *stmt = malloc(sizeof(Stmt));
            stmt->type = STMT_VAR_DECL;
            stmt->as.varDecl.varType = TOKEN_IDENTIFIER;  // Mark as custom type
            stmt->as.varDecl.isComptime = false;
            stmt->as.varDecl.isGlobal = isGlobal;
            strcpy(stmt->as.varDecl.structType, possibleType);
    ...
    // NEW: Check for destructuring assignment: (a, b, c) = expr
    if (check(parser, TOKEN_LPAREN)) {
        int startPos = parser->current;
        advanceToken(parser);  // consume '('
        
        // Try to parse as destructuring
        char varNames[MAX_PARAMS][256];
        int varCount = 0;
        bool isDestruct = true;
        ...
        if (check(parser, TOKEN_IDENTIFIER)) {
            while (true) {
                ...
                if (varCount < MAX_PARAMS) {
                    strcpy(varNames[varCount++], varToken->lexeme);
                }
            ...
            if (isDestruct && matchToken(parser, TOKEN_RPAREN) && matchToken(parser, TOKEN_ASSIGN)) {
                // It's destructuring!
                Stmt *stmt = malloc(sizeof(Stmt));
                stmt->type = STMT_DESTRUCT;
                stmt->as.destruct.varCount = varCount;
                for (int i = 0; i < varCount; i++) {
                    strcpy(stmt->as.destruct.varNames[i], varNames[i]);
                }
                stmt->as.destruct.value = parseExpression(parser);
                return stmt;
            }
        }
        
        // Not destructuring, reset and treat as expression
        parser->current = startPos;
    }
    
    if (check(parser, TOKEN_IDENTIFIER)) {
        Token *nameToken = advanceToken(parser);
        
        // NEW: Check for field assignment: obj.field = value
        if (matchToken(parser, TOKEN_DOT)) {
            Token *fieldToken = advanceToken(parser);
            
            // Check if it's a method call (will be handled as expression)
            if (check(parser, TOKEN_LPAREN)) {
                // This is a method call like obj.close(), treat as expression
                parser->current -= 3; // Back up past fieldToken, dot, and nameToken
                Stmt *stmt = malloc(sizeof(Stmt));
                stmt->type = STMT_EXPR;
            ...
            // It's field assignment
            consume(parser, TOKEN_ASSIGN, "Expected '=' after field name");
            Stmt *stmt = malloc(sizeof(Stmt));
            stmt->type = STMT_FIELD_ASSIGN;
            strcpy(stmt->as.fieldAssign.objectName, nameToken->lexeme);
            strcpy(stmt->as.fieldAssign.fieldName, fieldToken->lexeme);
            stmt->as.fieldAssign.value = parseExpression(parser);
            return stmt;
        }
...
Function parseFunction(Parser *parser) {
    ...
    // NEW: Handle struct definitions
    if (matchToken(parser, TOKEN_STRUCT)) {
        Token *nameToken = advanceToken(parser);
        
        StructType structType;
        strcpy(structType.name, nameToken->lexeme);
        ...
        while (!check(parser, TOKEN_RBRACE) && !isAtEndParser(parser)) {
            ...
            if (matchToken(parser, TOKEN_INT)) fieldType = TOKEN_INT;
            else if (matchToken(parser, TOKEN_DOUBLE)) fieldType = TOKEN_DOUBLE;
            else if (matchToken(parser, TOKEN_STRING_TYPE)) fieldType = TOKEN_STRING_TYPE;
            else if (matchToken(parser, TOKEN_BOOL)) fieldType = TOKEN_BOOL;
            else if (matchToken(parser, TOKEN_LIST)) fieldType = TOKEN_LIST;
            else if (matchToken(parser, TOKEN_FILE)) fieldType = TOKEN_FILE;
            else if (matchToken(parser, TOKEN_ERROR)) fieldType = TOKEN_ERROR;
            else if (matchToken(parser, TOKEN_TUPLE)) fieldType = TOKEN_TUPLE;
            else if (check(parser, TOKEN_IDENTIFIER)) {
                // Could be a nested struct type
                advanceToken(parser);
                fieldType = TOKEN_IDENTIFIER;
            } else {
                fprintf(stderr, "Parser Error: Expected field type in struct\n");
                exit(1);
            }
            
            Token *fieldName = advanceToken(parser);
            if (structType.fieldCount < MAX_STRUCT_FIELDS) {
                strcpy(structType.fields[structType.fieldCount].name, fieldName->lexeme);
                structType.fields[structType.fieldCount].type = fieldType;
                structType.fieldCount++;
            }
        }
        
        consume(parser, TOKEN_RBRACE, "Expected '}' after struct definition");
        
        registerStructType(&structType);
...
Function *findFunction(const char *name) {
    for (int i = 0; i < runtime.functionCount; i++) {
        if (strcmp(runtime.functions[i].name, name) == 0) {
            Function *func = &runtime.functions[i];
            
            // Check static scoping: static functions only accessible from same file
            if (func->isStatic) {
                // Get the file of the calling function (current stack frame)
                const char *callerFile = NULL;
                
                if (runtime.frameCount > 0) {
                    // We're inside a function call - check the caller's file
                    Function *callerFunc = runtime.frames[runtime.frameCount - 1].function;
                    if (callerFunc) {
                        callerFile = callerFunc->definedInFile;
                    }
                } else {
                    // Top-level code - use current file being parsed
                    callerFile = importTracker.currentFile;
                }
                
                // If static function is from different file than caller, skip it
                if (callerFile && strcmp(func->definedInFile, callerFile) != 0) {
                    continue;
                }
            }
...
Value evaluateCallExpr(CallExpr *call) {
    ...
    // NEW: Built-in: format(formatStr, args...)
    if (strcmp(call->name, "format") == 0) {
        if (call->argCount < 1) {
            fprintf(stderr, "Runtime Error: format() requires at least 1 argument\n");
            exit(1);
        }
        
        Value fmtVal = evaluateExpr(call->args[0]);
        if (fmtVal.type != VAL_STRING) {
            fprintf(stderr, "Runtime Error: format() first argument must be a string\n");
            exit(1);
        }
        ...
        for (int i = 0; fmtVal.as.stringVal[i] != '\0' && resultPos < MAX_STRING_LENGTH - 1; i++) {
            if (fmtVal.as.stringVal[i] == '{' && fmtVal.as.stringVal[i+1] == '}') {
                // Found placeholder
                if (argIndex < call->argCount) {
                    Value arg = evaluateExpr(call->args[argIndex++]);
                    char temp[256];
                    
                    switch (arg.type) {
                        case VAL_INT:
                            snprintf(temp, sizeof(temp), "%d", arg.as.intVal);
                            break;
                        case VAL_DOUBLE:
                            snprintf(temp, sizeof(temp), "%g", arg.as.doubleVal);
                            break;
                        case VAL_BOOL:
                            snprintf(temp, sizeof(temp), "%s", arg.as.boolVal ? "true" : "false");
                            break;
                        case VAL_STRING:
                            snprintf(temp, sizeof(temp), "%s", arg.as.stringVal);
                            break;
                        default:
                            snprintf(temp, sizeof(temp), "<value>");
                            break;
                    }
                    
                    int len = strlen(temp);
                    if (resultPos + len < MAX_STRING_LENGTH) {
                        strcpy(result + resultPos, temp);
                        resultPos += len;
                    }
                }
                i++; // Skip the '}'
            } else {
                result[resultPos++] = fmtVal.as.stringVal[i];
            }
        }
        result[resultPos] = '\0';
    ...
    // NEW: Built-in: open(path, mode)
    if (strcmp(call->name, "open") == 0) {
        if (call->argCount < 1 || call->argCount > 2) {
            fprintf(stderr, "Runtime Error: open() requires 1 or 2 arguments\n");
            exit(1);
        }
        
        Value pathVal = evaluateExpr(call->args[0]);
        if (pathVal.type != VAL_STRING) {
            fprintf(stderr, "Runtime Error: open() path must be a string\n");
            exit(1);
        }
        
        char mode[8] = "r";  // Default mode
        if (call->argCount == 2) {
            Value modeVal = evaluateExpr(call->args[1]);
            if (modeVal.type == VAL_STRING) {
                strncpy(mode, modeVal.as.stringVal, 7);
                mode[7] = '\0';
            }
        }
        
        return createFileValue(pathVal.as.stringVal, mode);
    }
    
    // NEW: File method: file.close()
    if (strcmp(call->name, "__close") == 0) {
        if (call->argCount < 1) {
            fprintf(stderr, "Runtime Error: close() requires a file object\n");
            exit(1);
        }
        
        Value fileVal = evaluateExpr(call->args[0]);
        if (fileVal.type != VAL_FILE) {
            fprintf(stderr, "Runtime Error: close() requires a file object\n");
            exit(1);
        }
        
        if (fileVal.as.fileVal.isOpen && fileVal.as.fileVal.handle) {
            fclose(fileVal.as.fileVal.handle);
            // Note: We can't modify the original value here, but in practice
            // the user should not use the file after closing
        }
        
        return createNullValue();
    }
    
    // NEW: File method: file.read()
    if (strcmp(call->name, "__read") == 0) {
        if (call->argCount < 1) {
            fprintf(stderr, "Runtime Error: read() requires a file object\n");
            exit(1);
        }
        ...
        if (!fileVal.as.fileVal.isOpen || !fileVal.as.fileVal.handle) {
            throwError("FileError", "File is not open", 1);
            return createStringValue("");
        }
        
        // Read entire file
        fseek(fileVal.as.fileVal.handle, 0, SEEK_END);
        long size = ftell(fileVal.as.fileVal.handle);
        fseek(fileVal.as.fileVal.handle, 0, SEEK_SET);
        ...
        size_t bytesRead = fread(buffer, 1, size, fileVal.as.fileVal.handle);
        buffer[bytesRead] = '\0';
        
        return createStringValue(buffer);
    }
    
    // NEW: File method: file.readln()
    if (strcmp(call->name, "__readln") == 0) {
        if (call->argCount < 1) {
            fprintf(stderr, "Runtime Error: readln() requires a file object\n");
            exit(1);
        }
        ...
        char buffer[MAX_STRING_LENGTH];
        if (fgets(buffer, MAX_STRING_LENGTH, fileVal.as.fileVal.handle)) {
    ...
    // NEW: File method: file.write(text)
    if (strcmp(call->name, "__write") == 0) {
        if (call->argCount < 2) {
            fprintf(stderr, "Runtime Error: write() requires file and text arguments\n");
            exit(1);
        }
        
        Value fileVal = evaluateExpr(call->args[0]);
        Value textVal = evaluateExpr(call->args[1]);
        
        if (fileVal.type != VAL_FILE) {
            fprintf(stderr, "Runtime Error: write() requires a file object\n");
            exit(1);
        }
        ...
        if (textVal.type == VAL_STRING) {
            strcpy(text, textVal.as.stringVal);
        } else if (textVal.type == VAL_INT) {
            snprintf(text, sizeof(text), "%d", textVal.as.intVal);
        } else if (textVal.type == VAL_DOUBLE) {
            snprintf(text, sizeof(text), "%g", textVal.as.doubleVal);
        } else if (textVal.type == VAL_BOOL) {
            snprintf(text, sizeof(text), "%s", textVal.as.boolVal ? "true" : "false");
        } else {
            strcpy(text, "");
        }
        
        fputs(text, fileVal.as.fileVal.handle);
        return createNullValue();
    }
    
    // NEW: File method: file.writeln(text)
    if (strcmp(call->name, "__writeln") == 0) {
        if (call->argCount < 2) {
            fprintf(stderr, "Runtime Error: writeln() requires file and text arguments\n");
            exit(1);
        }
        ...
        if (fileVal.type != VAL_FILE) {
            fprintf(stderr, "Runtime Error: writeln() requires a file object\n");
            exit(1);
        }
...
Value evaluateExpr(Expr *expr) {
    switch (expr->type) {
        ...
        case EXPR_STRUCT_INIT: {
            // NEW: Evaluate struct initialization
            Value structVal = createStructValue(expr->as.structInit.typeName);
            StructType *type = findStructType(expr->as.structInit.typeName);
            
            for (int i = 0; i < expr->as.structInit.fieldCount; i++) {
                Value fieldValue = evaluateExpr(expr->as.structInit.fieldValues[i]);
                // Find field index
                for (int j = 0; j < type->fieldCount; j++) {
                    if (strcmp(type->fields[j].name, expr->as.structInit.fieldNames[i]) == 0) {
                        structVal.as.structVal.fields[j] = fieldValue;
                        break;
                    }
                }
            }
        ...
        case EXPR_FIELD_ACCESS: {
            // NEW: Evaluate field access
            Value obj = evaluateExpr(expr->as.fieldAccess.object);
            Value *field = getStructField(&obj, expr->as.fieldAccess.fieldName);
            return *field;
        }
        
        case EXPR_TUPLE: {
            // NEW: Evaluate tuple
            Value elements[MAX_PARAMS];
            for (int i = 0; i < expr->as.tuple.count; i++) {
                elements[i] = evaluateExpr(expr->as.tuple.elements[i]);
            }
            return createTupleValue(elements, expr->as.tuple.count);
        }
    }
...
void executeStmt(Stmt *stmt) {
    ...
    switch (stmt->type) {
        ...
        case STMT_VAR_DECL: {
            ...
            if (stmt->as.varDecl.initializer) {
                ...
                if (stmt->as.varDecl.varType == TOKEN_LIST) {
                    val = createListValue();
                } else if (stmt->as.varDecl.varType == TOKEN_IDENTIFIER && stmt->as.varDecl.structType[0] != '\0') {
                    // NEW: Struct type - create instance with default values
                    val = createStructValue(stmt->as.varDecl.structType);
                } else {
                    val = createIntValue(0);
                }
            }
        ...
        case STMT_STRUCT_DEF:
            // NEW: Struct definitions are handled at parse time
            // Nothing to do at runtime
            break;
            
        case STMT_FIELD_ASSIGN: {
            // NEW: struct.field = value
            Value *obj = findVariable(stmt->as.fieldAssign.objectName);
            if (!obj || obj->type != VAL_STRUCT) {
                fprintf(stderr, "Runtime Error: '%s' is not a struct\n", stmt->as.fieldAssign.objectName);
                exit(1);
            }
            
            Value *field = getStructField(obj, stmt->as.fieldAssign.fieldName);
            *field = evaluateExpr(stmt->as.fieldAssign.value);
            break;
        }
        
        case STMT_TRY_CATCH: {
            // NEW: Try-catch block
            errorSystem.inTryBlock = true;
            errorSystem.tryDepth++;
            int oldErrorCount = errorSystem.count;
            
            executeStmt(stmt->as.tryCatch.tryBlock);
            
            errorSystem.tryDepth--;
            if (errorSystem.tryDepth == 0) {
                errorSystem.inTryBlock = false;
            }
            
            // If an error was thrown, execute catch block
            if (errorSystem.count > oldErrorCount) {
                ErrorValue err = errorSystem.errors[errorSystem.count - 1];
                Value errorVal = createErrorValue(err.type, err.message, err.code);
                setVariable(stmt->as.tryCatch.errorVar, errorVal);
                executeStmt(stmt->as.tryCatch.catchBlock);
                errorSystem.count--; // Clear the error
            }
            break;
        }
        ...
        case STMT_THROW: {
            // NEW: Throw error
            Value errorVal = evaluateExpr(stmt->as.throwStmt.errorExpr);
            if (errorVal.type == VAL_ERROR) {
                throwError(errorVal.as.errorVal.type, 
                          errorVal.as.errorVal.message, 
                          errorVal.as.errorVal.code);
            } else {
                throwError("Error", "Thrown value is not an error", 1);
            }
            break;
        }
        ...
        case STMT_DESTRUCT: {
            // NEW: Destructuring assignment
            Value val = evaluateExpr(stmt->as.destruct.value);
            
            if (val.type == VAL_TUPLE) {
                for (int i = 0; i < stmt->as.destruct.varCount && i < val.as.tupleVal.count; i++) {
                    setVariable(stmt->as.destruct.varNames[i], val.as.tupleVal.elements[i]);
                }
            } else if (val.type == VAL_LIST) {
                for (int i = 0; i < stmt->as.destruct.varCount && i < val.as.listVal.count; i++) {
                    setVariable(stmt->as.destruct.varNames[i], val.as.listVal.elements[i]);
                }
            } else {
                fprintf(stderr, "Runtime Error: Can only destructure tuples or lists\n");
                exit(1);
            }
            break;
        }
...
void parseFile(const char *filename) {
    ...
    // Set current file for static scoping
    strncpy(importTracker.currentFile, filename, MAX_PATH_LENGTH - 1);
    importTracker.currentFile[MAX_PATH_LENGTH - 1] = '\0';
    ...
    while (!isAtEndParser(&parser)) {
        ...
        if (func.name[0] != '\0') {
            // Set which file defined this function
            strncpy(func.definedInFile, importTracker.currentFile, MAX_PATH_LENGTH - 1);
            func.definedInFile[MAX_PATH_LENGTH - 1] = '\0';
...
int main(int argc, char **argv) {
    if (argc != 2) {
        fprintf(stderr, "Usage: ./main filename.c2\n");
        return 1;
    }
